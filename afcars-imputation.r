...
for(i in 1:length(files)){
  ...
  data<-mice::complete(imp.out, 
                       ...
                       include = TRUE)
  cnty_entries<-data%>%
    group_by(.imp, STATE, FIPSCODE, year, HISORGIN)%>%
    summarise(abuse = sum(abuse, na.rm=TRUE), 
              first_entry = sum(first_entry, na.rm=TRUE),
              reun_exit = sum(reun_exit, na.rm=TRUE),
              caseload = n())%>%
    rename(FIPS=FIPSCODE)%>%
    ungroup()
  filename<-paste("imputed_county", i, ".csv", sep="")
  write_csv(cnty_entries, filename)
}
